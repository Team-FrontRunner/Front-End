    ...
    <div style={styles.container}>
      {gameState === 'START' && (
        <div style={{ textAlign: 'center', marginTop: '130px' }}>
          ...
          <button 
            onClick={startGame}
            style={{ width: '280px', padding: '25px', backgroundColor: '#84CC16', color: 'white', borderRadius: '60px', border: 'none', fontSize: '28px', fontWeight: '900', boxShadow: '0 8px 0 #65A30D' }}
          >
            시작하기 🚀
          </button>
        </div>
      ...
      {gameState === 'PLAYING' && question && (
        <>
            ...
            {question.options.map((opt, idx) => (
              <button 
                ...
                style={styles.optionBtn(
                  ...
                  (feedback && selectedIdx === idx && opt !== question.answer)
                )}
          ...
          {feedback && (
            <div style={{ textAlign: 'center', marginTop: '10px', fontSize: '22px', fontWeight: '900', color: feedback === 'correct' ? '#16A34A' : '#EF4444' }}>
              {feedback === 'correct' ? '정답입니다! 잘하셨어요!' : feedback === 'timeout' ? '시간이 다 됐어요!' : `정답은 ${question.answer} 입니다.`}
            </div>
          )}
        </>
      )}
      ...
      {gameState === 'RESULT' && (
        <div style={{ textAlign: 'center', marginTop: '100px' }}>
          ...
          <div style={{ backgroundColor: '#FFF', padding: '40px 20px', borderRadius: '40px', marginBottom: '50px', boxShadow: '0 10px 20px rgba(0,0,0,0.05)' }}>
            ...
            <h2 style={{ fontSize: '64px', fontWeight: '900', color: '#84CC16' }}>{score * 10}점</h2>
            <p style={{ fontSize: '18px', color: '#9CA3AF' }}>10문제 중 {score}문제 정답!</p>
          </div>
          <button 
            onClick={startGame}
            style={{ width: '280px', padding: '22px', backgroundColor: '#374151', color: 'white', borderRadius: '50px', border: 'none', fontSize: '24px', fontWeight: '900' }}
          >
            다시 하기 🔄
          </button>
        </div>
